...
pub struct CfgCellInfo {
    ...
    /// Individual (point-to-point) half-duplex call hangtime in seconds: how long an active
    /// simplex individual call stays up with NOBODY holding the floor before the BS releases it
    ...
    /// Default: 13 seconds (just when field radios give such a call up themselves). Range: 0–300.
...
pub struct CellInfoDto {
    ...
    /// Simplex individual call hangtime in seconds. 0 = disabled. Default: 13.
...
pub fn cell_dto_to_cfg(ci: CellInfoDto) -> CfgCellInfo {
    CfgCellInfo {
        ...
        individual_hangtime_secs: ci.individual_hangtime_secs.unwrap_or(13).clamp(0, 300),
        call_timeout_secs: {
